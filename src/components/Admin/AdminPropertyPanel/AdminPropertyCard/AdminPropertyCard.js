...
function AdminPropertyCard({prop}) {
    ...
    const approvalHandler = (ad) => {
        fetch( process.env.REACT_APP_SERVER_URL + "/approve?id=" + ad._id + "&approval=" + !isApproved, {
            method: "PUT",
            headers: tokenHeader()
        }).then(res => {
    ...
    return (
        <div className="property-card">
            ...
            <div className="property-admin-controls">
                ...
                {isApproved ? <RiCheckboxCircleFill onClick={() => approvalHandler(property)}/> : <RiCheckboxCircleLine onClick={() => approvalHandler(property)}/>}
